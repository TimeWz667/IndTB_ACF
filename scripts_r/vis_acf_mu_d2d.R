...
cost_items <- setNames(cost$Item, cost$Item)

cost_fn <- lapply(cost_items, function(item) {
  x = cost %>% filter(Item == item)
  if (x$std <= 0) {
    return(function(n) {rep(0, n)})
  } else {
    return(function(n) rnorm(n, x$M, x$std))
  }
})
